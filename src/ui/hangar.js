...
export function renderHangar(container, { selectedId, wingman, round, onLaunch }) {
  container.innerHTML = `
    ...
    <p class="dialog-description">PILOT 1 · F-15C EAGLE — current aircraft<br>PIXY · Select your supporting aircraft.</p>
    ${wingman ? `<p class="hangar-status">CURRENT PIXY: ${wingman.alive ? `${Math.ceil(wingman.hp)} / ${wingman.definition.maxHP} HP` : 'DESTROYED'}</p>` : ''}
    <fieldset class="encounter-choices aircraft-choices"><legend>PIXY AIRCRAFT</legend>
    ...
    <p class="dialog-tip">${round ? 'Airframe condition, ammunition and score carry forward. Switching PIXY to a different aircraft supplies a full-health replacement. Keeping the same aircraft preserves its damage or loss.' : 'Your wingman follows, engages and defends itself using local combat AI.'}</p>
  ...
  button.onclick = async () => {
    ...
    container.querySelector('#hangar-feedback').textContent = 'Preparing PIXY…';
