...
Completed: M1, M2, M3, M4, M5, M6. Current: **M7-A — Wingman Audio Architecture**.
Pending: **M7-B — Prerecorded Wingman Audio Asset Integration**.
...
real prerecorded Pixy MP3 lines from validated manifest pools, applies radio
effects in Web Audio at runtime, and stays silent while M7-A pools are empty.
M7-B adds the actual files. Chatterbox and neural TTS are superseded and are not
on the active roadmap unless a later explicit instruction restores them.
...
`src/audio.js`. M7-B will populate prerecorded real Pixy lines.
...
M7-A adds procedural chat mumble and an empty prerecorded gameplay-radio scaffold.
...
M7-A supplies procedural chat mumble, ATTACK/REGROUP controls and an empty
prerecorded gameplay-radio scaffold. M7-B adds real MP3 files and human
listening validation. M8 microphone/STT remains future scope. Private source
recordings remain outside this public repository. The canonical persona remains
`docs/wingman/lorebook.md`, unchanged. See `docs/m7a-validation.md`.
...
25. Wingman Audio Architecture (M7-A)
...
effects. Empty pools are a normal silent state until M7-B. The game remains
playable without the LLM or prerecorded assets.
...
The immediate development priority is M7-A — Wingman Audio Architecture; M7-B
will add prerecorded MP3 assets and listening validation. M1 — Architecture Foundation, M2 — Endless Round
Core, M3 — Encounter Selection, M4 — Wingman Flight and Combat AI, scoped
M5 — Hangar and Aircraft Selection, and M6 — Wingman Text Conversation are
completed and must remain working. No M8 microphone/STT is authorized.
